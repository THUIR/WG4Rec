...
class SogouDP(HistoryDP):
    ...
    def __init__(self, graph_l, graph_sample, word_wn, word_tn, nb_type, nb_topic, max_his, sparse_his, *args,
                 **kwargs):
    ...
    def get_item(self, index):
        ...
        c_search_his = result[C_HISTORY_NEG]
        c_search_his = utils.pad_array(c_search_his, self.max_his, 0)
        result[C_HISTORY] = np.concatenate([c_history, c_search_his, [result[IID]]])
        text_dicts = [self.data_reader.doc_dict] * self.max_his + \
                     [self.data_reader.url_dict] * self.max_his + \
                     [self.data_reader.doc_dict]
        result[C_SENT] = np.array(
            [text_dicts[i][C_SENT][iid] for i, iid in enumerate(result[C_HISTORY])], dtype=np.int64)
        result[C_SENT_LENGTH] = np.array(
            [text_dicts[i][C_SENT_LENGTH][iid] for i, iid in enumerate(result[C_HISTORY])], dtype=np.int64)
        result[C_WORD_GRAPH] = result[C_SENT].copy()
        return result
    ...
    def get_batch(self, batch, skip_keys=None, info_keys=None):
        ...
        if skip_keys is None:
            skip_keys = []
        if self.graph_l > 0:
            feed_dict = HistoryDP.get_batch(self, batch, skip_keys=skip_keys + [C_WORD_GRAPH],
                                            info_keys=info_keys + [C_SENT_LENGTH])
            rank_n = len(batch[0])
