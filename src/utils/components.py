...
def qk_attention(query, key, value, valid=None):
    """
    :param query: ? * l * a
    :param key: ? * l * a
    :param value: ? * l * v
...
def scaled_dot_product_attention(q, k, v, scale=None, valid=None):
    """
    ...
    """
    ele_valid = 1 if valid is None else valid.float()  # ? * L_q * L_k
    ...
    att_exp = att_v.exp() * ele_valid  # ? * L_q * L_k
    att_sum = att_exp.sum(dim=-1, keepdim=True)  # ? * L_q * 1
    sum_valid = 1 if valid is None else ele_valid.sum(dim=-1, keepdim=True).gt(0).float()  # ? * L_q * 1
    att_w = att_exp / (att_sum * sum_valid + 1 - sum_valid)  # ? * L_q * L_k
    context = torch.matmul(att_w, v)  # ? * L_q * V
    return context
