...
def group_user_interactions_csv(in_csv, out_csv, pos_neg, label=LABEL, sep='\t', seq_sep=','):
    all_data = pd.read_csv(in_csv, sep=sep)
    group_inters = group_user_interactions_df(in_df=all_data, label=label, pos_neg=pos_neg, seq_sep=seq_sep)
    group_inters.to_csv(out_csv, sep=sep, index=False)
    return group_inters


def group_user_interactions_df(in_df, pos_neg, label=LABEL, seq_sep=','):
    all_data = in_df
...
def random_split_data(all_data_file, dataset_name, vt_ratio=0.1, copy_files=None, copy_suffixes=None):
    ...
    # print(test_set)
    train_set.to_csv(os.path.join(dir_name, dataset_name + TRAIN_SUFFIX), index=False, sep='\t')
    validation_set.to_csv(os.path.join(dir_name, dataset_name + VALIDATION_SUFFIX), index=False, sep='\t')
    test_set.to_csv(os.path.join(dir_name, dataset_name + TEST_SUFFIX), index=False, sep='\t')
...
def leave_out_by_time_df(all_df, leave_n=1, warm_n=5, split_n=1, max_user=-1):
    ...
    if min_label > 0:
        ...
        leave_df = []
        if warm_n > 0:  # 如果warm_n <= 0的画不需要保证测试集warm
        ...
        for i in range(split_n):
            ...
            split_dfs.append(split_df)
    if len(leave_df) > 0:
        train_df = pd.concat([leave_df, all_df]).sort_index()
    else:
        train_df = all_df
    return train_df, split_dfs


def leave_out_by_time_csv(all_data_file, dataset_name, leave_n=1, warm_n=5, max_user=-1, u_f=None, i_f=None):
    """
    ...
    train_set, (test_set, validation_set) = leave_out_by_time_df(
        all_data, warm_n=warm_n, leave_n=leave_n, split_n=2, max_user=max_user)
